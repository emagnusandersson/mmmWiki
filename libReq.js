...
// Packages held back: The below packages requires node 20 or later, but the default node.js-package in the apt repository is only on 18.
// serialize-javascript 7.0.2 is held back on version 6.0.2
// mongodb 7.0.0 is held back on version 6.9
...
// Images should include last-mod-time
// slide show could be made to work with object-fit: contain 
...
// parse async
// toggle-buttons should be dark-grey when on (with green checkbox)
// toggle-buttons should have "on"- resp "off"- text (green for on)

// (?<!cmd|del|set|get)redis

// idplace-tester: login via facebook doesn't work
...
// mmmWiki-style buttons should be used on all apps
// pm2 ondemand
// FF webpush
// form with labels
// ios button should be gray
// weakMap prestandatest
// helpbutton with z-index

// Graphicsmagick command line
// stackoverflow change question
// distribute python program
// neuron time

// On firefox stale pages are served
// PopUp-views: blanket and center should change place like in Buvt

// 20260329 One should be able to filter on pages with a certain sub-page or certain image
// 20260401 Test mongodb update event
// 20260401 IdChild should probably be called IdChildNTemplate
//          IdChildLax should probably be called IdChildNTemplateWStubs
...
// permission-button should be green when secure ( indicators-chars overstricken when permission is false)
...
app.reqBU=async function(strArg) {
  ...
  if(req.method=='POST'){
    ...
    //let objUrl=new URL("http://trash.com"+req.url);
    //let objQS=objUrl.searchParams, qs=objQS.toString()
    var qs=objUrl.search.slice(1)
    try{
      jsonInput=decodeQueryParam(qs); // Translates "+" to " ", I don't think one can use URL to any greate success, because everything after the question mark is json
    }catch(e){
      console.error(e);
      reqMy.out400(Error("decodeQueryParam error"));
      return
    }
  } 
  ...
  if(boFilter){
    var {arrName}=inObj;
      // Check input
    if(!(arrName instanceof Array)) { reqMy.out500(Error('arrName is not an Array')); debugger;  return; }
    var boOk=true; for(var s of arrName) { if(typeof s=='string' && s.length); else {boOk=false; break;};}
    if(!boOk) { reqMy.out500(Error('at least one of the entries in arrName is not a string')); debugger;  return;}
  } else { var arrName=[]; }
...
let makeOutput=function(objOut, strHtmlText){
  ...
  var {objSiteDefault, objSite, objPage, boPageExist=true}=objOut, {pageName}=objPage; //, {www:wwwSite}=objSite;
  ...
  if(!boPageExist || !objPage.boSiteMap || !objPage.boOR || boTemplate){ strMetaNoIndex='<meta name="robots" content="noindex">\n'; }
...
app.reqIndex=async function() {
  ...
  var boSecFetch=typeof strT != 'undefined'
  if(boSecFetch){
    var strT=req.headers.get('sec-fetch-mode');
    if(!(strT=='navigate' || strT=='same-origin')) { reqMy.out400(`sec-fetch-mode header not allowed (${strT})`); return;}
  }

  
  // try {
  //   var pathName = decodeQueryParam(pathName); // Translates "+" to " ", I don't think one can use URL to any greate success, because everything after the question mark is json
  // } catch (e) {
  //   console.error(e);
  //   reqMy.out400(`decodeQueryParam error`);
  //   return;
  // }
  ...
  var [err,tmp]=await setRedis(sessionIDStrict+'_Strict', 1, maxAdminRUnactivityTime);
  ...
  if(objPage==null) { // No such page 
    ...
    var objOut={ CSRFCode:'',   boTalkExist:false, strEditText:"", arrVersionCompared:[null,1], matVersion:{}, objSiteDefault, objSite, objPage, boPageExist:false};
    var strHtmlText='';
  ...
  var arrRet=[null];
    ...
    if(tMod<=tModCache && strHashParse.length==32) {  // Meta data (but not strHtmlText) has changed
      ...
      if(!objFileHtml) {arrRet=[new Error("!objFileHtml")]; break stuff;}
      ...
      var {IdChild:IdChildOld, StrImage:StrImageOld, IdParent}=objPage;
...
app.reqStatic=async function() {
  ...
  if(err) {
    ...
    if(err.code=='EISDIR') {reqMy.out400('EISDIR'); return;}
...
app.reqMediaImageThumb=async function(){
  ...
  if(width==0 && height==0){
    ...
    //var {width, height}=probeImageSize.sync(bufDataOrg)
    var [err, {width, height}] = await probeImageSize(bufDataOrg).toNBP(); if(err) {reqMy.out500(err); return; };
  }
  ...
  //if(scale>=1) { scale=1;  if(wNew>100){ reqMy.out400('Bad Request, 100px is the max width for enlargements.'); return;} } // No enlargements
