...
ReqBE.prototype.go=async function(){ 
  ...
  if(req.method=='POST'){
    ...
    try{
      ...
      console.error(e);
      this.mesEO(Error("decodeQueryParam error"));
      return
    }
  ...
  if(!('sessionIDStrict' in reqMy.cookies)) {this.mesEO('no sessionIDStrict cookie'); return}
  var sessionIDStrict=reqMy.cookies.sessionIDStrict;
  var [err, val]=await evalRedis(luaGetNExpire, [sessionIDStrict+'_Strict'], [maxAdminRUnactivityTime]);
...
ReqBE.prototype.myChMod=async function(inObj){    
  ...
  var {Id}=inObj;
    // Check input
  if(!(Id instanceof Array) || Id.length==0) { return [new ErrorClient('!(Id instanceof Array) || Id.length==0')];}
  var boOk=true; for(var s of Id) { if(typeof s=='string' && s.length); else {boOk=false; break;};}
  if(!boOk) { return [new ErrorClient('At least one input is not a string')]; }
...
ReqBE.prototype.pageLoad=async function(inObj) {
  ...
  stuff:
  {
    ...
    if(rev>=lenRev) {
      ...
      //res.setHeaderMy(oH); reqMy.out400("no such version");
...
ReqBE.prototype.renamePage=async function(inObj){ 
  ...
  stuff:
  {
    ...
    if(objPage===null) {
      arrRet=[Error(`idPageOld: ${idPageOld}, siteName: ${siteName}, pageNameOldLC: ${pageNameOldLC}`)]; break stuff;
    }
...
//                             loadFrFilesOrBufs
//                            /        |        \
//                parseZipFile  parseCSVMyWType  storePageMultFrBU
// 
...
app.loadFrFilesOrBufs=async function(FileOrg){
  ...
  for(var key in FileCsv) {
    ...
    var [err, Result]=await parseCSVMyWType(fileCsv); if(err) return [err];
...
app.parseCSVMyWType=async function(oFile){
  var {strPath, buf}=oFile;
  if(typeof buf=='undefined'){
    var [err, buf]=await fsPromises.readFile(strPath).toNBP();    if(err) return [err];
  }
  ...
  //var [arrHead,arrData]= parseCSVMy0WithSeparateHeadHandling(strCSV);  // arrData is an array of arrays of strings.
  var arrAll= parseCSVMy(strCSV);  
