...
ReqBE.prototype.go=function*(){ 
  ...
  if('x-requested-with' in req.headers){
    ...
    console.error('x-requested-with not set, url: '+req.url+", method: "+req.method+", x-type: "+req.headers['x-type']); 
    this.mesEO(Error("x-requested-with not set"));
    return;
  }
...
app.loadMetaFrBU=function*(flow, oFile){
  ...
  if(typeof strCSV=='undefined'){
    //var strFileLong=path.join(__dirname, '..', 'mmmWikiBackUp', strFile);
  ...
  if(strFile=='site.csv'){
    ...
    for(var j=0;j<nRow;j++){
      arrDataN[j]=arrArrange(arrData[j],[ICol.boOR, ICol.boOW, ICol.boSiteMap, ICol.tCreated, ICol.tMod, ICol.tMod, ICol.siteName, ICol.pageName]);
    }
    var Val=[].concat.apply([], arrDataN);
    ...
    var arrDataN=Array(nRow);
    for(var j=0;j<nRow;j++){
      arrDataN[j]=arrArrange(arrData[j],[ICol.pageNameLC, ICol.url, ICol.tMod, ICol.nAccess, ICol.tLastAccess, ICol.siteName]);
    }
    var Val=[].concat.apply([], arrDataN);
  }
...
app.loadFrBUOnServ=function*(flow, StrFile){ 
  var strFileToLoadFolder=path.join(__dirname, '..', 'mmmWikiBackUp'); 
  ...
  if( StrFile.length==1) {
    var pathTmp=path.join(__dirname, '..', 'mmmWikiBackUp',StrFile[0]); 
