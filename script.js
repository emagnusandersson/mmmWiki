...
extend(app, {http, path, fsPromises, myCrypto, zlib, gzip, pipeline, JSZip, serialize, validator, mime, ejs, mongodb, mongoSanitize, Buffer, probeImageSize }); //, url
...
import * as libA from './lib.js'; libA.blah();
import * as libB from './libServerGeneral.js'; libB.blah();
import * as libC from './libServer.js'; libC.blah();
import * as libD from './lib/foundOnTheInternet/lcs.js'; libD.blah(); 
import * as libE from './lib/foundOnTheInternet/diff.js'; libE.blah();
import * as libF from './myDiff.js'; libF.blah();
...
//var [err,tmp]=await cmdRedis('GET',[keyR]); if(err) {console.error(err); Deno.exit(-1);}
var [err,tmp]=await getRedis(keyR); if(err) {console.error(err); Deno.exit(-1);}

//var [err,tmp]=await cmdRedis('GET',[keyR]);
const boNewConfig=strMd5Config!==tmp; 
//if(boNewConfig) { let [err,tmp]=await cmdRedis('SET',[keyR,strMd5Config]);   if(err) {console.error(err); Deno.exit(-1);}      }
if(boNewConfig) { let [err,tmp]=await setRedis(keyR,strMd5Config);   if(err) {console.error(err); Deno.exit(-1);}      }
...
var keyR=strAppName+'_IndexTemplateHash';
...
var [err, boHashTemplateMatch]=await evalRedis(luaIndexTemplateHashTestNSetFun, [keyR], [strHashTemplate]);
...
class ReqMy{
  ...
  out400=function(str="400 Bad Request\n"){ this.clearHeaders().outCode(400, str); }
  out500=function(e){
    debugger
...
const handler=async function(){
  ...
  let objUrl=new URL(uTmp), qs=objUrl.search;
  ...
  if(typeof isRedirAppropriate!='undefined'){ 
    //let uTmp=isRedirAppropriate(req); if(uTmp) { reqMy.out301(uTmp); return; }
    let uTmp=isRedirAppropriate(objUrl); if(uTmp) { reqMy.out200(`The domain name has changed, use: ${uTmp} instead`); return; }
  }

    // Set boTLS
  let boTLS=false;
  if(boHeroku || boAF){
    const strTmp=req.headers.get('x-forwarded-proto')
  ...
  pathNameOrg=decodeURIComponent(pathNameOrg)
  let wwwReq=domainName+pathNameOrg;
  
  let wwwSite=domainName, pathName=pathNameOrg;
  if(!wwwSite) console.error(Error("!wwwSite"));
  ...
  var [err, intCount]=await evalRedis(luaDDosCounterFun, [keyR_DDOS], [tDDOSBan]);
    // Write to response
  reqMy.outCookies.sessionIDDDos=sessionIDDDos+strCookiePropNormal

    // Update keyR_DDOSIP counter
  let ipClient=getIP(reqMy), keyR_DDOSIP=ipClient+'_DDOS';
  var [err, intCountIP]=await evalRedis(luaDDosCounterFun, [keyR_DDOSIP], [tDDOSIPBan]);
  ...
  extend(reqMy, {objUrl, objQS, strSchemeLong, boTLS, wwwSite, pathName}); 
  ...
  //else if(pathName=='/'+leafManifest){   req.boSiteDependant=1; await reqStatic.call(objReqRes);   }
    ...
    reqMy.end(`<script>let tserver=${tserver}, tclient=(new Date()).valueOf(); console.log('tserver: '+tserver/1000);console.log('tclient: '+tclient/1000);console.log('tdiff: '+(tclient-tserver)/1000);</script>`);
  }
