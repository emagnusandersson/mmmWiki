...
app.getInfoNDataNoReqRes=async function(objArg){
  ...
  if(results.length==0) { extend(Ou, {mess:'wwwNotFound'}); return [null,Ou]; } //reqMy.out500(wwwSite+', site not found'); return [];
  ...
  var [err, results]=await this.myMySql.query(sql, Val); if(err) return [err];
  if(results.length) { 
    //reqMy.out301(results[0].urlRedirDomain+'/'+queredPage); return;
  ...
  var [err, results]=await this.myMySql.query(sql, Val); if(err) return [err];
  if(results.length==0) { extend(Ou, {mess:'noDefaultSite'}); return [null,Ou]; } //reqMy.out500('no default site'); return;
  ...
  if(boRedirectCase) { extend(Ou, {mess:'redirectTLS', boTLS:site.boTLS});  return [null,Ou]; } //reqMy.out301(pageRedir);  return;
  ...
  if(nVersion==0) { Ou.mess='noSuchRev'; return [null,Ou];  } //reqMy.out500('no versions?!?'); return;
  if(rev>=nVersion) { Ou.mess='noSuchRev'; return [null,Ou];  } //reqMy.out500(`No rev ${rev}, (${nVersion} version)`); return; 
  ...
  if(boValidReqCache) { Ou.mess='304'; return [null,Ou]; } //reqMy.out304(); return;
