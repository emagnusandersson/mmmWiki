...
//declare global {var ucfirst, pad2, calcLabel, decodeQueryParam, extractLoc, extract, isUpperCase, endsWith, str_repeat, strCountEqualFromStart}
...
// Math
//
//declare global {var isNumber, sign, bound, closest2Val, preferedValue, numWithMetricPrefix, numWithMetricPrefixArr } 
...
app.numWithMetricPrefix=function(n, boRound=true, boStringOutput=true){ 
  var Pre=['', 'k', 'M', 'G', 'T', 'P', 'E', 'Z', 'Y', 'R', 'Q'];
  // var Pre=['', 'k'];
  var pre='', nCalc=n, boPreFound=false
  for(var i in Pre){
    if(nCalc<1000) {pre=Pre[i]; boPreFound=true; break;}
    nCalc=nCalc/1000;
  }
  if(!boPreFound) nCalc=n;
  if(boRound) nCalc=Math.round(nCalc);
  var arrO; if(pre.length) arrO=[nCalc, pre]; else arrO=[nCalc];
  if(boStringOutput) return arrO.join('');
  return [nCalc, pre]
}
app.numWithMetricPrefixArr=function(N){var l=N.length, StrOut=Array(l);for(var i=0;i<l;i++){ StrOut[i]=numWithMetricPrefix(N[i]); } return StrOut; }



app.numWithBinaryPrefix=function(n, boRound=true, boStringOutput=true){ 
  var Pre=['', 'Ki', 'Mi', 'Gi', 'Ti', 'Pi', 'Ei', 'Zi', 'Yi', 'Ri', 'Qi'];
  // var Pre=['', 'Ki'];
  var pre='', nCalc=n, boPreFound=false
  for(var i in Pre){
    if(nCalc<1024) {pre=Pre[i]; boPreFound=true; break;}
    nCalc=nCalc/1024;
  }
  if(!boPreFound) nCalc=n;
  if(boRound) nCalc=Math.round(nCalc);
...
//var o=numWithBinaryPrefix(1234); debugger
